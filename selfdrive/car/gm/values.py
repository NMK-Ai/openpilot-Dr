...
class CarControllerParams:
  STEER_MAX = 300  # الحد الأقصى لعزم التوجيه لـ GM هو 3Nm. يُستخدم بواسطة carcontroller لتوليد مخرجات LKA
  STEER_STEP = 3  # إطارات التحكم النشطة لكل أمر (~33hz)
  INACTIVE_STEER_STEP = 10  # إطارات التحكم غير النشطة لكل أمر (10hz)
  STEER_DELTA_UP = 10  # معدلات التغيير تتطلب مراجعة بسبب ضعف EPS الملحوظ
  STEER_DELTA_DOWN = 25
  STEER_DRIVER_ALLOWANCE = 50
  STEER_DRIVER_MULTIPLIER = 4
  STEER_DRIVER_FACTOR = 100
  NEAR_STOP_BRAKE_PHASE = 0.5  # م/ث

  # نبضات التحكم لعرض "خدمة مثبت السرعة المتكيف" و "خدمة الكاميرا الأمامية" على لوحة القيادة
  ADAS_KEEPALIVE_STEP = 100
  CAMERA_KEEPALIVE_STEP = 100

  # السماح بهامش صغير تحت -3.5 م/ث^2 من معيار ISO 15622:2018 لأننا
  # نقوم بالتحكم في الحلقة المغلقة، وقد نحتاج إلى تطبيق المزيد من الكبح
  # إذا كنا على منحدر.
  # يجب أن يحافظ نظام التحكم لدينا على متوسط لمدة 2 ثانية أعلى من
  # -3.5 م/ث^2 كما هو محدد في حدود المخطط.
  ACCEL_MAX = 2.  # م/ث^2
  ACCEL_MIN = -4.  # م/ث^2

  def __init__(self, CP):
    # جداول البحث للغاز/الفرامل
    self.ZERO_GAS = 2048  # التوقف بدون تسارع أو تباطؤ
    self.MAX_BRAKE = 400  # حوالي -4.0 م/ث^2 مع استخدام الطاقة المتجددة (الريجين)

    if CP.carFingerprint in CAMERA_ACC_CAR:
      self.MAX_GAS = 3400
      self.MAX_ACC_REGEN = 1514
      self.INACTIVE_REGEN = 1554
      # المركبات المزودة بنظام ACC المستند إلى الكاميرا لا تستخدم الطاقة المتجددة أثناء التفعيل.
      # تتحول الكاميرا إلى MAX_ACC_REGEN من ZERO_GAS وتستخدم الفرامل الاحتكاكية على الفور.
      max_regen_acceleration = 0.

    else:
      self.MAX_GAS = 3072  # حد الأمان، وليس الحد الأقصى لنظام ACC. نظام ACC الأصلي >4096 من وضعية التوقف التام.
      self.MAX_ACC_REGEN = 1404  # الحد الأقصى للطاقة المتجددة لنظام ACC أقل قليلاً من الحد الأقصى للطاقة المتجددة عبر المقابض.
      self.INACTIVE_REGEN = 1404
      # محركات الاحتراق الداخلي (ICE) لديها قوة فرملة أقل بكثير مقارنةً بالطاقة المتجددة في السيارات الكهربائية،
      # الحد الأدنى الأقل يزيل بعض المنطقة الميتة للكبح.
...
class CAR:
  HOLDEN_ASTRA = "هولدن أسترا RS-V BK 2017"
  VOLT = "شيفروليه فولت بريميير 2017"
  CADILLAC_ATS = "كاديلاك ATS Premium Performance 2018"
  MALIBU = "شيفروليه ماليبو بريميير 2017"
  ACADIA = "جي إم سي أكاديا دينالي 2018"
  BUICK_LACROSSE = "بويك لاكروس 2017"
  BUICK_REGAL = "بويك ريجال إسينس 2018"
  ESCALADE = "كاديلاك إسكاليد 2017"
  ESCALADE_ESV = "كاديلاك إسكاليد ESV 2016"
  BOLT_EUV = "شيفروليه بولت EUV 2022"
  SILVERADO = "شيفروليه سيلفرادو 1500 2020"
  EQUINOX = "شيفروليه إكوينوكس 2019"
  TRAILBLAZER = "شيفروليه ترايلبليزر 2022"
...
# فولت بريميير 2018 مع برنامج ثابت محدث، بدون رادار
  {
...
# لاكروس بريميوم AWD 2017
  ...
  CAR.BUICK_REGAL : [
# ريجال تور إكس إسينس مع نظام ACC 2018
  {
  ...
  CAR.CADILLAC_ATS: [
# كاديلاك ATS كوبيه بريميوم بيرفورمانس 3.6L دفع خلفي مع نظام ACC 2018
  {
  ...
  CAR.MALIBU: [
# ماليبو بريميير مع نظام ACC 2017
  {
  ...
  CAR.ACADIA: [
# أكاديا دينالي مع نظام ACC 2018
  {
...
# نحن مدمجون عند الكاميرا مع نظام VOACC في هذه السيارات (بدلاً من ASCM مع حزام OBD-II)
