...
class UserSoftDisableAlert(SoftDisableAlert):
  def __init__(self, alert_text_2: str):
    ...
    self.alert_test_1 = _("سيتم فصل القائد الآلي")
...
def below_steer_speed_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  return Alert(
    _("المساعدة في القيادة غير متاحة أدنى من %s") % get_display_speed(CP.minSteerSpeed, metric),
    "",
...
def calibration_incomplete_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  first_word = _('إعادة المعايرة') if sm['liveCalibration'].calStatus == log.LiveCalibrationData.Status.recalibrating else _('معايرة')
  return Alert(
    f"{first_word} قيد المعايرة: {sm['liveCalibration'].calPerc:.0f}%",
...
def posenet_invalid_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  ...
  return NoEntryAlert(msg, alert_text_1=_("سرعة بوزنت غير صالحة"))
...
def low_memory_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  return NormalPermanentAlert(_("الذاكرة ممتلئة"), f"{sm['deviceState'].memoryUsagePercent}% مستخدم")
...
def modeld_lagging_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  return NormalPermanentAlert(_("تأخر في نموذج القيادة"), f"{sm['modelV2'].frameDropPerc:.1f}% تم تجاوز الإطارات")
...
EVENTS: Dict[int, Dict[str, Union[Alert, AlertCallbackType]]] = {
  ...
  EventName.startupNoControl: {
    ...
    ET.NO_ENTRY: NoEntryAlert(_("وضع الداشكام")),
  },
  ...
  EventName.vehicleModelInvalid: {
    ...
    ET.SOFT_DISABLE: soft_disable_alert(_("فشل في التعرف على معلمات السيارة")),
  },
  ...
  EventName.parkBrake: {
    ET.USER_DISABLE: EngagementAlert(AudibleAlert.disengage),
    ET.NO_ENTRY: NoEntryAlert(_("تم تفعيل الجلنط ")),
  },
  ...
  EventName.soundsUnavailable: {
    ET.PERMANENT: NormalPermanentAlert(_("السماعات غير موجودة"), _("أعد تشغيل الجهاز")),
    ET.NO_ENTRY: NoEntryAlert(_("تأكد من تثبيت الجهاز بأمان")),
  },
  ...
  EventName.calibrationRecalibrating: {
    ET.PERMANENT: calibration_incomplete_alert,
    ET.SOFT_DISABLE: soft_disable_alert("اكتشاف إعادة تثبيت الجهاز: إعادة المعايرة"),
    ET.NO_ENTRY: NoEntryAlert("اكتشاف إعادة التثبيت: إعادة المعايرة"),
  },
  ...
  EventName.controlsdLagging: {
    ...
    ET.NO_ENTRY: NoEntryAlert(_("تأخر في عملية التحكم: أعد تشغيل الجهاز الخاص بك")),
  },
  ...
  EventName.canError: {
    ...
    ET.NO_ENTRY: NoEntryAlert(_("خطأ CAN: قم بفحص الاتصالات")),
  },

  EventName.canBusMissing: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert(_("قاطع اتصال الشبكة الكان (CAN)")),
    ET.PERMANENT: Alert(
      _("الاتصال بشبكة الكان قد انقطع: الكابل قد يكون به خلل"),
      "",
      AlertStatus.normal, AlertSize.small,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, 1., creation_delay=1.),
    ET.NO_ENTRY: NoEntryAlert(_("انقطاع اتصال الشبكة الكن (CAN): قم بفحص الاتصالات")),
  },
  ...
  EventName.lkasDisabled: {
    ET.PERMANENT: NormalPermanentAlert(_("تم تعطيل نظام مساعدة الحفاظ على المسار (LKAS): أعد تشغيل السيارة")),
    ET.NO_ENTRY: NoEntryAlert(_("نظام مساعدة الحفاظ على المسار (LKAS) معطل")),
  },

  EventName.vehicleSensorsInvalid: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert(_("مستشعرات السيارة غير صالحة")),
    ET.PERMANENT: NormalPermanentAlert(_("معايرة مستشعرات السيارة"), _("قد السيارة للمعايرة")),
    ET.NO_ENTRY: NoEntryAlert(_("معايرة مستشعرات السيارة"), _("قد السيارة للمعايرة")),
  },

  # dp - use for manual lane change
  EventName.manualSteeringRequiredBlinkersOn: {
    ET.PERMANENT: Alert(
      _("التوجيه مطلوب: قم بتشغيل الإشارة"),
      "",
      AlertStatus.normal, AlertSize.small,
